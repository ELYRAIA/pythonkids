  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950">
      ...
      <div className="w-full px-4 py-8 text-center">
        <div className="flex flex-wrap justify-center gap-2 mb-4">
          ...
          <div className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-bold border ${
            ...
            ⏳ {daysLeft > 0 ? `J-${daysLeft}` : "Saison terminée"}
          </div>
        </div>
        <h1 className="text-3xl sm:text-4xl font-extrabold text-white mb-2">
          Pass de <span className="bg-gradient-to-r from-yellow-400 to-orange-500 bg-clip-text text-transparent">Combat</span>
        </h1>
        <p className="text-sm text-gray-400 mb-6">Gagne de l'XP en apprenant et débloque des récompenses exclusives !</p>
        ...
        <div className="max-w-lg mx-auto bg-slate-800/80 rounded-2xl p-5 border border-slate-700 mb-4">
          <div className="flex items-center justify-between mb-3">
            <div className="text-center">
              ...
              <div className="text-xs text-gray-500">Niveau actuel</div>
            </div>
            <div className="flex-1 mx-4">
              ...
              {currentLevel < BP_MAX_LEVEL && (
                <div className="text-xs text-gray-500 mt-1 text-center">{xpInfo.xpToNext} XP pour le niveau {currentLevel + 1}</div>
              )}
            </div>
            <div className="text-center">
        ...
        <div className="max-w-lg mx-auto grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4 text-xs text-gray-400">
          {[
            { label: "Leçon", xp: 50 }, { label: "Défi", xp: 75 },
            { label: "Quête", xp: 100 }, { label: "Duel gagné", xp: 100 },
          ].map((src) => (
        ...
        {!state.isPremium && (
          <div className="flex flex-col items-center gap-2">
            <p className="text-sm text-gray-400">
              Accède à toutes les récompenses premium pour{" "}
            ...
            <div className="w-72">
              ...
              >
                <PayPalButtons
                  ...
                  onError={() => setPaypalError("Erreur PayPal, réessaie.")}
                />
              </PayPalScriptProvider>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 mb-4 flex gap-4 text-xs text-gray-500">
        <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-slate-600" /> Gratuit</div>
        <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded bg-gradient-to-r from-yellow-500 to-orange-500" /> Premium</div>
        <div className="flex items-center gap-1.5"><span className="text-green-400">✓</span> Réclamé</div>
        <div className="flex items-center gap-1.5"><span>🔒</span> Verrouillé</div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 flex flex-col gap-1">
                <div className="text-[10px] text-gray-600 font-semibold uppercase tracking-wide">Gratuit</div>
                {freeReward && (
              ...
              <div className="flex-1 flex flex-col gap-1">
                <div className={`text-[10px] font-semibold uppercase tracking-wide ${state.isPremium ? "text-yellow-500" : "text-gray-600"}`}>
                  {state.isPremium ? "✨ Premium" : "🔒 Premium"}
                </div>
                {premiumReward && (
                  <div className="flex items-center gap-2">
                    ...
                    {isUnlocked && !state.isPremium && (
                      <span className="text-[10px] text-gray-600 italic">Premium requis</span>
                    )}
                  </div>
                )}
              </div>
