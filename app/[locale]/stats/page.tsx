    ...
    <div className={`min-h-screen ${mounted ? "fade-in" : "invisible"}`}>
      ...
      <div className="w-full px-6 py-8 max-w-2xl mx-auto space-y-6">
        ...
        <div className="text-center">
          <h1 className="text-2xl font-extrabold text-gray-800 dark:text-white mb-1">{t("title")}</h1>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-5">
          <h2 className="text-sm font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-4">{t("xp_rank")}</h2>
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {xpInfo.nextRank ? (
                ...
                <p className="text-xs text-yellow-500 dark:text-yellow-400 mt-1 font-semibold">🌟 {t("max_rank")}</p>
              )}
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wide">{t("calendar_title")}</h2>
            <div className="flex items-center gap-3 text-xs">
              <span className="text-gray-400 dark:text-slate-500">{t("record", { days: streak.longestStreak })}</span>
              <span className="text-orange-500 font-bold">{t("current_streak", { days: streak.currentStreak })}</span>
            </div>
          </div>
                ...
                {week.map((day, di) => (
                  <div
                    ...
                    className={`h-3.5 rounded-sm transition-all ${
                      ...
                      day.active ? "bg-orange-400 dark:bg-orange-500" : "bg-gray-100 dark:bg-slate-700"
                    }`}
                  />
                ))}
              </div>
          ...
          <div className="flex items-center gap-2 mt-3 justify-end">
            <span className="text-[10px] text-gray-400 dark:text-slate-500">{t("less_activity")}</span>
            {["bg-gray-100 dark:bg-slate-700", "bg-orange-200 dark:bg-orange-800", "bg-orange-400 dark:bg-orange-500"].map((cls, i) => (
              <div key={i} className={`w-3 h-3 rounded-sm ${cls}`} />
            ))}
            <span className="text-[10px] text-gray-400 dark:text-slate-500">{t("more_activity")}</span>
          </div>
        </div>

        {/* Progression par niveau */}
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-5">
          <h2 className="text-sm font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-4">{t("levels_title")}</h2>
        ...
        <div className="bg-gradient-to-br from-purple-600 to-pink-500 rounded-2xl p-5 text-white text-center">
          ...
          <p className="text-purple-100 text-sm">{t("estimated_time")}</p>
        </div>
