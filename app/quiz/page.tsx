        ...
        {phase === "list" && (
          <>
            <div className="mb-6">
              ...
              <h1 className="text-2xl font-extrabold text-gray-800 dark:text-white mt-2">📚 Quiz de révision</h1>
              <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">
                Révise les leçons que tu as terminées et gagne des gemmes !
              </p>
            </div>
            ...
            {available.length === 0 ? (
              <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 text-center border border-slate-200 dark:border-slate-700">
                ...
                <p className="text-base font-bold text-gray-700 dark:text-slate-200">Pas encore de quiz disponible</p>
                <p className="text-sm text-gray-400 dark:text-slate-500 mt-1">Termine des leçons pour débloquer leurs quiz !</p>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wide">
                        Niveau {entry.levelId} — {entry.levelName}
                      </p>
                      <p className="text-sm font-bold text-gray-800 dark:text-white truncate mt-0.5">{entry.lessonTitle}</p>
                      <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">
                        {entry.questions.length} question{entry.questions.length > 1 ? "s" : ""} · +{entry.questions.length * 5} 💎 max
                      </p>
                    </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-md overflow-hidden">
            ...
            <div className={`bg-gradient-to-r ${active.levelColor} px-5 py-4`}>
              <div className="flex items-center justify-between mb-1">
                <button onClick={() => setPhase("list")} className="text-white/70 hover:text-white text-xs">✕ Quitter</button>
            ...
            <div className="p-5">
              ...
              <div className="flex justify-end">
                {!confirmed ? (
                  ...
                  >
                    {current + 1 < active.questions.length ? "Suivant →" : "Voir mon score"}
                  </button>
                )}
              </div>
            </div>
        ...
        {phase === "result" && active && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-md p-8 text-center">
            ...
            <h2 className="text-xl font-extrabold text-gray-800 dark:text-white mb-1">
              {starsFinal === 3 ? "Parfait !" : starsFinal === 2 ? "Bien joué !" : "Continue !"}
            </h2>
            <p className="text-sm text-gray-500 dark:text-slate-400 mb-4">
              {scoreFinal}/{active.questions.length} bonne{scoreFinal > 1 ? "s" : ""} réponse{scoreFinal > 1 ? "s" : ""}
            </p>
            ...
            <div className="bg-teal-50 dark:bg-teal-900/20 rounded-xl px-5 py-3 mb-6 inline-block">
              ...
              <p className="text-xs text-teal-500 dark:text-teal-500">gemmes gagnées</p>
            </div>
