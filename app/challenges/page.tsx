...
export default function ChallengesPage() {
  ...
  // "Pour toi" = minLevel dans [playerLevel - 1 .. playerLevel + 1]
      ...
      <div className="w-full px-6 py-8 max-w-5xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mb-6">
            Résous les défis, gagne des points, débloque des badges !
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            ...
            {mounted && (
              <div className="inline-flex items-center gap-2 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-full px-4 py-2 text-sm text-purple-700 dark:text-purple-300 font-semibold">
                ...
                <span>Ton niveau : {LEVEL_NAMES[playerLevel]}</span>
              </div>
            )}
            <Link
              href="/mistakes"
        ...
        <div className="flex gap-2 mb-4 max-w-lg mx-auto">
          ...
          <button
            ...
            title={sort === "defaut" ? "Trier : non faits en premier" : "Trier : ordre par défaut"}
          ...
          >
            {sort === "non_faits" ? "✓ Non faits" : "Trier"}
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-8">
          {(["Pour toi", "Tous", "Facile", "Moyen", "Difficile"] as const).map((f) => (
            ...
            >
              {f === "Pour toi"
                ? `⭐ Pour toi (${CHALLENGES.filter((c) => isForMe(c.minLevel)).length})`
                : f === "Tous"
                ? `Tous (${total})`
        ...
        {visible.length === 0 && (
          <div className="text-center py-16 text-gray-400 dark:text-slate-500">
            ...
            <p className="font-bold">Tous les défis de ce niveau sont terminés !</p>
            ...
            const card = (
              <div className={`group relative rounded-2xl border-2 p-5 h-full transition-all ${
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {/* Badge niveau recommandé */}
                  <span className="text-xs text-gray-400 dark:text-slate-500">
                    · {LEVEL_EMOJIS[challenge.minLevel]} Niv. {challenge.minLevel}
                  </span>
                </div>
                ...
                <p className="text-xs text-gray-500 dark:text-slate-400 leading-relaxed line-clamp-2">
                  {isLocked
                    ? `Débloque le niveau ${challenge.minLevel} (${LEVEL_NAMES[challenge.minLevel]}) pour accéder à ce défi.`
                ...
                {isDone && bestTime && (
                  <div className="mt-3 flex items-center gap-1.5 text-xs text-green-600 dark:text-green-400 font-semibold">
                    <span>⏱</span>
                    <span>Meilleur temps : {formatTime(parseInt(bestTime))}</span>
                  </div>
                )}
